...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const radius = Math.round(Math.min(440, Math.max(180, w * 0.16)));
  ...
  return (
    <section
      ref={sectionRef}
      id="top"
      className="relative min-h-[92vh] sm:min-h-screen flex flex-col justify-between pt-24 sm:pt-28 pb-8 px-6 md:px-12 overflow-hidden bg-white cursor-pointer"
    >
      {/* Cursor-Following Floating Audio Player Badge */}
      <MusicCursorToggle targetRef={sectionRef} />

      {/* 1. Desktop Dual-Image Interactive Spotlight Reveal Background */}
      <div className="hidden lg:block absolute inset-0 pointer-events-none z-0 overflow-hidden transform-gpu" aria-hidden="true">
        {/* Base Layer: Image 1 */}
        <div
      ...
      {/* 2. Refined, Minimalist Typography Layout */}
      <motion.div
        style={{ y: contentY, opacity: contentOpacity }}
        className="max-w-[1400px] mx-auto w-full relative z-10 my-auto py-4"
      >
        <div className="max-w-md sm:max-w-lg">
          ...
          {/* Micro Category Tag */}
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/70 backdrop-blur-md border border-black/10 text-[10px] sm:text-xs font-mono font-semibold uppercase tracking-widest text-black/70 mb-3">
            <Sparkles size={11} className="text-red" />
            <span>Web &amp; Shopify Developer</span>
          </div>

          {/* Elegant Proportional Headline */}
          ...
          <p className="text-xs sm:text-sm text-black/75 leading-relaxed max-w-sm sm:max-w-md font-normal bg-white/30 backdrop-blur-xs p-1.5 rounded-lg">
            Hi, I&apos;m <strong className="text-black font-semibold">Vikash Choudhary</strong>. I help modern brands scale with lightning-fast e-commerce stores, custom Next.js web applications, and precision UI engineering — built for speed, designed to convert.
          </p>
      ...
      <div className="relative mt-4 border-t border-b border-black/10 py-3 bg-white/85 backdrop-blur-md overflow-hidden z-10">
        <div className="flex whitespace-nowrap animate-marquee">
          {[...WORDS, ...WORDS, ...WORDS].map((w, i) => (
            <span
              key={i}
              className="font-display text-xl sm:text-2xl font-bold px-6 flex items-center gap-4 text-black/20"
            >
